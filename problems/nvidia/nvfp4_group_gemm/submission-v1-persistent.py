# Persistent group GEMM kernel v3
# Architecture follows the NVIDIA grouped blockscaled GEMM blog:
# - init_tensormap_from_atom: OUTSIDE while loop (done once)
# - update_tensormap: INSIDE while loop (conditional on group change)
# - C tile: pointer arithmetic with fixed (128, 128) shape
...
threads_per_cta = 128
num_acc_stage = 1
num_ab_stage = 4  # Reference uses 5-8; 4 stages safely fits in B200 shared memory
...
def kernel(
    ...
    # ══════════════════════════════════════════════════════════════
    # Global tensor partitions (fixed TMA shapes — outside loop)
    # ══════════════════════════════════════════════════════════════
    ...
    # ══════════════════════════════════════════════════════════════
    # KEY FIX from blog: Initialize tensormaps OUTSIDE while loop
    # Blog: "init_tensormap_from_atom" done by MMA warp BEFORE loop
    # ══════════════════════════════════════════════════════════════
    if warp_idx == 0:
    ...
    # ══════════════════════════════════════════════════════════════
    # PERSISTENT: Grid-stride loop over all tiles
    # ══════════════════════════════════════════════════════════════
    # KEY FIX from blog: use cutlass.Int32(-1) for type consistency
    last_group_idx = cutlass.Int32(-1)
    tile_idx = cta_id

    while tile_idx < total_tiles:
        # ── Delinearize tile_idx → (group_idx, coord_x, coord_y) ──
        group_idx = 0
        find = False
        coord_x = 0
        coord_y = 0
        ...
        # ── Construct C tile with FIXED shape ──
        c_base_addr = tensor_of_abc_ptrs[group_idx, 2]
        tile_m_start = cutlass.Int32(coord_x) * cutlass.Int32(mma_tiler_mnk[0])
        tile_n_start = cutlass.Int32(coord_y) * cutlass.Int32(mma_tiler_mnk[1])
        c_elem_offset = tile_m_start * n + tile_n_start
        ...
        c_tile_layout = cute.make_layout(
            ...
            stride=(cute.assume(n, 32), 1),
        )
        gC_tile = cute.make_tensor(c_tile_ptr, c_tile_layout)
        tCgC = thr_mma.partition_C(gC_tile)

        # ── Update tensormap (blog pattern: conditional on group change) ──
        # KEY FIX from blog: only update_tensormap inside loop, NOT init
        ...
        # KEY FIX from blog: update last_group_idx OUTSIDE the if block
        last_group_idx = group_idx

        # ── K-tile count ──
        k_tile_cnt = cute.ceil_div(k, mma_tiler_mnk[2])

        # ── Slice TMA globals to this tile's coordinates ──
        ...
        cC = cute.make_identity_tensor((mma_tiler_mnk[0], mma_tiler_mnk[1]))
...
def my_kernel(
    ...
    tensor_of_tensormap = cute.make_tensor(
        ...
        cute.make_layout((num_sms, 4, 16), stride=(64, 16, 1)),
    )
