# Making the kernel with persistent scheduling
...
num_sms = 128  # Blackwell B200
grid = (num_sms, 1, 1)

# Passing total tile counts to kernel
...
def kernel(
    ...
    * (1.0 / (1.0 + cute.math.exp(-x, fastmath=True))),
    ...
    total_tiles = total_tiles_m * total_tiles_n * total_tiles_l
    #
    ...
    while tile_idx < total_tiles:
        tile_l = tile_idx // (total_tiles_m * total_tiles_n)
        tile_mn = tile_idx % (total_tiles_m * total_tiles_n)
        tile_n = tile_mn // total_tiles_m
        tile_m = tile_mn % total_tiles_m
        
        mma_tile_coord_mnl = (tile_m, tile_n, tile_l)
        
        # ═══════════════════════════════════
        # SLICE TENSORS FOR THIS TILE
        # ═══════════════════════════════════
        tAgA_tile = tAgA[(None, tile_m, None, tile_l)]
        tBgB1_tile = tBgB1[(None, tile_n, None, tile_l)]
        tBgB2_tile = tBgB2[(None, tile_n, None, tile_l)]
        tAgSFA_tile = tAgSFA[(None, tile_m, None, tile_l)]
        tBgSFB1_tile = tBgSFB1[(None, tile_n, None, tile_l)]
        tBgSFB2_tile = tBgSFB2[(None, tile_n, None, tile_l)]
        ...
        if warp_idx == 0:
            ...
            for k_tile_idx in range(k_tile_cnt):
                ...
                cute.copy(
                    tma_atom_sfa,
                    tAgSFA_tile[(None, ab_empty.count)],
                    tAsSFA[(None, ab_empty.index)],
                    tma_bar_ptr=ab_empty.barrier,
                )
                cute.copy(
                    tma_atom_sfb1,
                    tBgSFB1_tile[(None, ab_empty.count)],
                    tBsSFB1[(None, ab_empty.index)],
                    tma_bar_ptr=ab_empty.barrier,
                )
                cute.copy(
                    tma_atom_sfb2,
                    tBgSFB2_tile[(None, ab_empty.count)],
    ...
    #
         #  Epilogue inside tile loop
    # Partition for epilogue
    #
        tTR_gC_tile = tTR_gC[(None, None, None, None, tile_m, tile_n, tile_l)]
        ...
        cute.copy(simt_atom, tTR_rC, tTR_gC_tile)
        
        acc_full.release()
        
        # INCREMENT TILE INDEX FOR GRID-STRIDE LOOP
        tile_idx += total_ctas
...
def my_kernel(
    ...
    * (1.0 / (1.0 + cute.math.exp(-x, fastmath=True))),
    ...
    # Passing total tile counts to kernel
    total_tiles_m = cute.ceil_div(m, mma_tiler_mnk[0])
    total_tiles_n = cute.ceil_div(n, mma_tiler_mnk[1])
    total_tiles_l = l
    # Launch the kernel.
    kernel(
